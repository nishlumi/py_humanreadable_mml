...
class TextWrapper:
    ...
    def split_text(self, text):
        # 英単語、日本語文字、句読点などを分割
        pattern = r'([ぁ-んァ-ン一-龥ーｰ～－、。；：！＆＾＿？（）「」]|[０１２３４５６７８９]|[ａｂｃｄｅｆｇｈｉｊｋｌｍｎｏｐｑｒｓｔｕｖｗｘｙｚＡＢＣＤＥＦＧＨＩＪＫＬＭＮＯＰＱＲＳＴＵＶＷＸＹＺ]|[a-zA-Z0-9]+|[\/,.^_~#:;!?@&%<>\-]|\s+)'
...
class GameUI:
    ...
    def set_direct_round(self, label, ui):
        self.roundui[label] = self.get_object(ui)
...
class GUIText(GameUI):
    ...
    def draw(self):
        ...
        if self.blink_time > 0:
            if pyxel.frame_count % self.blink_time == 0:
                pyxel.dither(0.5)
...
class GUIRect(GameUI):
    ...
    def draw(self):
        ...
        if self.filled:
            ...
            pyxel.rectb(self.bounds.x, self.bounds.y, self.bounds.w, self.bounds.h, self.color)
...
class GUICheckbox(GameUI):
    def __init__(self, text, x, y, chked: bool = False, font=None, color1=pyxel.COLOR_BLACK, color2=pyxel.COLOR_BLACK, shifted_x = 0, shifted_y = 0):
        ...
        self.check_color = pyxel.COLOR_WHITE
    ...
    def update(self):
        if pyxel.btnp(pyxel.MOUSE_BUTTON_LEFT):
            if self.check_touch_area(pyxel.mouse_x, pyxel.mouse_y):
                self.pressed =  True
    ...
    def draw(self):
        ...
        if self.checked:
            ...
            pyxel.rect(self.bounds.x, self.bounds.y, self.bounds.w, self.bounds.h, self.check_color)
        else:
            ...
            pyxel.rectb(self.bounds.x, self.bounds.y, self.bounds.w, self.bounds.h, self.check_color)
        self.text.draw()
...
class GUIDialog(GameUI):
    ...
    def __init__(self, x, y, w, h, btns = BTN_OK):
        ...
        self.dialog_color = pyxel.COLOR_BLACK
    ...
    def recalc_text(self, index):
        self.contents[index].bounds.w = self.bounds.w
        self.contents[index].recalc_wrap()
    
    def check_pressed(self):
        """To check press which button 
            return {bool} OK, YES, NO
        """
        ret = {"OK":False, "YES": False, "NO": False}
        if self.btntype == self.BTN_OK:
            ret["OK"] = self.buttons[0].pressed
        elif self.btntype == self.BTN_YESNO:
            ret["YES"] = self.buttons[0].pressed
            ret["NO"] = self.buttons[1].pressed
    ...
    def draw(self):
        if self.is_drawend:
            pyxel.dither(0.1)
            pyxel.rect(0, 0, pyxel.width, pyxel.height, pyxel.COLOR_WHITE)
            pyxel.dither(1.0)
            pyxel.rect(self.bounds.x, self.bounds.y, self.bounds.w, self.bounds.h, self.dialog_color)
            for u in self.contents:
                u.draw()
            for button in self.buttons:
                button.draw()
        else:
            pyxel.rect(self.bounds.x, self.bounds.y, pos(self.curdraw), self.bounds.h, self.dialog_color)
